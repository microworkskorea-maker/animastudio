    ...
    try {
      const res = await fetch('/api/backend/voice/generate', {
        ...
        body: JSON.stringify({
          ...
          dialogues: scenes.map(s => ({ sceneNumber: s.sceneNumber, text: s.dialogue })),
          settings: { engine: 'elevenlabs', voiceId: voice, voiceLabel: VOICES.find(v=>v.id===voice)?.label, speed: 1.0, emotion: 'neutral', subtitleStyle: 'white-outline', subtitlePosition: 'bottom' }
        })
      })
  ...
  return (
    <div className="max-w-3xl mx-auto">
          ...
          <div className="flex gap-2 flex-wrap">
            {VOICES.map(v => (
              <button key={v.id} onClick={() => setVoice(v.id)}
                className={`px-4 py-2 rounded-full text-sm border transition-all ${voice === v.id ? 'bg-forest-100 border-forest-500 text-forest-700 font-medium' : 'border-cream-300 text-ink-500 hover:border-forest-300'}`}>
                {v.label}
              </button>
            ))}
          </div>
      ...
      <div className="flex justify-end gap-3">
        <button onClick={() => router.push('/video')} className="px-5 py-2.5 rounded-xl border border-cream-300 text-sm text-ink-600 hover:bg-cream-100">← 영상 생성</button>
        <button onClick={() => router.push('/export')} className="px-5 py-2.5 rounded-xl bg-forest-600 text-white text-sm font-medium hover:bg-forest-700">내보내기 →</button>
      </div>
    </div>
  )
}
