...
{{
  "scenes": [
    {{
      ...
      "cameraNote": "카메라 앵글/이동 지시 (예: 클로즈업, 줌아웃 등)"
    }}
  ],
...
2. 대사는 해당 장면 길이({inp.duration // scene_count}초)에 말할 수 있는 분량으로 제한
3. 마지막 장면에는 반드시 브랜드/제품 클로징이 포함되어야 합니다
4. JSON 이외의 텍스트는 절대 출력하지 마세요
""".strip()
...
async def generate_scenario(inp: ScenarioInput):
    ...
    try:
        ...
        try:
            ...
            # JSON 블록 추출 시도 (모델이 마크다운으로 감쌌을 때)
            import re
